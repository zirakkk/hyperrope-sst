...
*HyperRoPE-SST consistently outperforms CNN-based models (Conv2D, Conv3D, SSRN) and transformer-based methods (LSGA-ViT, HiT, SpectralFormer, SQSFormer) across all metrics. Detailed per-class accuracies and confusion matrices are available in the `results/` directory.*
