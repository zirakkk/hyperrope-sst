...
class HiT(nn.Module):
    def __init__(self, params):
        ...
        self.layers = net_params.get("layers", [4, 3, 14, 3])
        self.img_size = data_params.get("patch_size", 15)
        self.patch_size = 3
        self.in_chans = data_params.get("spectral_size", 200)
        self.num_classes = data_params.get("num_classes", 13)
        self.embed_dims = data_params.get("embed_dims", [400, 400, 512, 512])
        self.transitions = net_params.get("transitions", [False, True, False, False])
        self.segment_dim = net_params.get("segment_dim", [8, 8, 4, 4])
        self.mlp_ratios = net_params.get("mlp_ratios", [3, 3, 3, 3])
        self.skip_lam = 1.0
        self.qkv_bias = False
        self.qk_scale = None
        self.drop_rate = 0.1
        ...
        for i in range(len(self.layers)):
            stage = basic_blocks(self.embed_dims[i], i, self.layers, self.segment_dim[i], mlp_ratio=self.mlp_ratios[i],
                                 qkv_bias=self.qkv_bias, qk_scale=self.qk_scale, attn_drop=self.attn_drop_rate, drop_path_rate=self.drop_path_rate,
                                 norm_layer=self.norm_layer, skip_lam=self.skip_lam, mlp_fn=self.mlp_fn)
            network.append(stage)
            if i >= len(self.layers) - 1:
                break
            if self.transitions[i] or self.embed_dims[i] != self.embed_dims[i + 1]:
                patch_size = 2 if self.transitions[i] else 1
                network.append(Downsample(self.embed_dims[i], self.embed_dims[i + 1], patch_size))
        ...
        self.conv_cls_head = nn.Linear(368, self.num_classes)
